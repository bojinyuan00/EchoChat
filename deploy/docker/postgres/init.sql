...
-- ============================================================
-- Phase 2e-2 会议 MVP（设计文档 §5.1）
--   meeting_rooms         会议房间
--   meeting_participants  参与者记录
--   meeting_chats         会议内文字聊天
-- 所有时间字段沿用项目风格 TIMESTAMP(0)（而非设计草案的 TIMESTAMPTZ），与
-- auth_users / im_messages 等表保持一致；Go 侧 model 搭配 gorm:"type:timestamp(0)"。
-- ============================================================
...
-- 主持人 + 状态的列表查询（如"我主持的进行中会议"）
CREATE INDEX idx_meeting_rooms_host_status ON meeting_rooms (host_id, status, created_at DESC);
-- 会议号快速查找（唯一约束已自动建索引，冗余索引已移除；入会查询走 UNIQUE）

-- ============================================================
-- meeting_participants: 参与者记录（出入场完整轨迹）
--   ON DELETE CASCADE：room 删除时自动清理参与者
--   UNIQUE(room_id, user_id)：同一用户在同一房间只有一条记录（重入仅更新 left_at=NULL）
-- ============================================================
...
-- ============================================================
-- meeting_chats: 会议内文字聊天（独立于 im_messages）
--   清理策略：每日扫描 meeting_rooms.status=2 AND ended_at < NOW() - INTERVAL '24 hours'
--   批量 DELETE FROM meeting_chats WHERE room_id IN (...)
-- ============================================================
