...
-- 旧数据库升级：2026-04-25 新增 inviter_id 字段（群邀请改为 pending 流程）
-- 已有库里若该列不存在则补齐，完全幂等
ALTER TABLE im_group_join_requests
    ADD COLUMN IF NOT EXISTS inviter_id BIGINT DEFAULT NULL;
...
CREATE INDEX IF NOT EXISTS idx_group_join_req_inviter ON im_group_join_requests(inviter_id);
